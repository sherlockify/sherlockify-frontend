...
const reqUrl = 'https://run-sherlock-4rsz5rrjca-uw.a.run.app'
...
function App() {
  ...
  return (
    <ChakraProvider>
      <Flex py={16} px={16} flexDirection="column" gap={10} bgColor="#1A2B34">
        <LandingPage onRequest={() => setSites([])}
          onResponse={(data) => setSites(prevData => [...prevData, data])} />
        <Stack gap={2}>
          <Text color={"gray.300"}><i>{(sites.filter((s) => s.site.toLowerCase().includes(siteSearch))).length} results found</i></Text>
          <SearchSites setSiteSearch={setSiteSearch} />
        </Stack>
        <WebsiteCards sites={sites.filter((s) => s.site.toLowerCase().includes(siteSearch))} />
      </Flex>
    </ChakraProvider>
  );
}
